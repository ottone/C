...
//  
//  Modificate il programma per la valutazione delle espressioni in notaizone polacca inversa dell'Eserizio 12.13, in modo che possa elaborare degli oprandi interi 
//  maggiori di nove. 
// 
//
...
void convertToPostfix(char infix[], char postfix[]){ // converte la notaizone infissa in quella polacca
	...
	while(!(isEmpty(stack))){		// Fintanto che la pila non è vuota , leggete infix da sinistra a destra ed eseguite le seguenti operazioni:
		...
		if(isdigit(infix[i])){	//Nel caso in cui il carattere corrente di infix sia un numero copiatelo nell'elemento successivo di postfix
			while(isdigit(infix[i])) *(ptr++) = infix[i++];
			*(ptr++) = ' ';	
